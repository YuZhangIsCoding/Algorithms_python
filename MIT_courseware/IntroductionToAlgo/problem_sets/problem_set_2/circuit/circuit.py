...
class PriorityQueue:
    ...
    def append(self, key):
        """Inserts an element in the priority queue and maintain heap invariant."""
        if key is None:
            raise ValueError('Cannot insert None in the queue')
        self.queue.append(key)
##        self._shiftdown(0, len(self.queue)-1)

    def _shiftdown(self, startpos, pos):
        '''same function used in the python source code
        '''
        ## useing newitem could reduce the time used to swap values.
    ...
    def min_heapify(self, i):
        '''Correct a single violations.

        Pay attention to the index difference here, python starts indexing with 0
        while the 
        '''
        l = 2*i+1
        r = l+1
        if l < len(self.queue) and self.queue[i] > self.queue[l]:
            smallest = l
        else:
            smallest = i
        if r < len(self.queue) and self.queue[smallest] > self.queue[r]:
            smallest = r
        if smallest != i:
    ...
    def build_min_heap(self):
        ...
        Actually not used in this problem because the append method already did this.
        '''
        i = len(self.queue)/2
        while i >= 0:
            self.min_heapify(i)
