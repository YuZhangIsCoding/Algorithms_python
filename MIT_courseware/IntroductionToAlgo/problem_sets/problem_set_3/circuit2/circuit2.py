...
class BSTNode(object):
    ...
    def delete(self):
        if self.left and self.right:
            temp = self.next_larger()
            temp.key, self.key = self.key, temp.key
            return temp.delete()
        else:
...
class BST(object):
    ...
    def next_larger(self, k):
        node = self.find(k)
        return node and node.next_larger(node)
    
    def insert(self, k):
        '''Need to return the node in case we want to rebalance'''
        node = self.nodeclass(None, k)
        if self.root:
    ...
    def delete(self, k):
        ...
        if node is self.root:
            dummy = self.nodeclass(None, None)
            self.root.parent = dummy
...
class AVL(BST):
    ...
    def rebalance(self, node):
        while node:
            ...
            AVL._update_rank(node)
            if AVL._height(node.left) > 1+AVL._height(node.right):
                if AVL._height(node.left.left) < AVL._height(node.left.right):
                    self.left_rotate(node.left)
                self.right_rotate(node)
            elif AVL._height(node.left) < -1+AVL._height(node.right):
    ...
    def _update_height(node):
        '''Use this with _height equals to add an attribute height externally
        '''
    ...
    def _rank(node):
        if node:
            ...
            return 0
    @staticmethod
    def _update_rank(node):
        node.rank = sum((AVL._rank(node.left), AVL._rank(node.right)))+1
class RangeIndex(AVL):
  ...
  def list(self, first_key, last_key):
    ...
    results = []
    self.Node_list(lca, first_key, last_key, results)
    return results

  def Node_list(self, node, l, h, results):
      if not node:
          return
      if l <= node.key <= h:
          results.append(node.key)
      if node.key >= l:
          self.Node_list(node.left, l, h, results)
      if node.key <= h:
          self.Node_list(node.right, l, h, results)

  def LCA(self, l, h):
      node = self.root
      while node and (l > node.key or node.key > h):
          if l < node.key:
              node = node.left
          else:
              node = node.right
      return node
  def Rank(self, node):
      mysum = AVL._rank(node.left)+1
      while node.parent:
          if node.parent.right is node:
              mysum += AVL._rank(node.parent.left)+1
          node = node.parent
      return mysum
  def count(self, first_key, last_key):
    """Number of keys that fall within [first_key, last_key]."""
    node1 = self.find(first_key)
    node2 = self.find(last_key)
    if not node2:
        self.add(last_key)
        node2 = self.find(last_key)
        if not node2:
            pdb.set_trace()
        temp = self.Rank(node2)
    ...
    if not node1:
        self.add(first_key)
        node1 = self.find(first_key)
        ans = temp-self.Rank(node1)
        self.remove(first_key)
        return ans
    else:
        return temp-self.Rank(node1)+1
